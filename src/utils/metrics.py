...
def ndcg_(pos_index, pos_len):
    ...
    iranks = np.zeros_like(pos_index, dtype=np.float16)
    iranks[:, :] = np.arange(1, pos_index.shape[1] + 1)
    idcg = np.cumsum(1.0 / np.log2(iranks + 1), axis=1)
    for row, idx in enumerate(idcg_len):
        idcg[row, idx:] = idcg[row, idx - 1]

    ranks = np.zeros_like(pos_index, dtype=np.float16)
...
def map_(pos_index, pos_len):
    ...
    sum_pre = np.cumsum(pre * pos_index.astype(np.float16), axis=1)
    len_rank = np.full_like(pos_len, pos_index.shape[1])
    actual_len = np.where(pos_len > len_rank, len_rank, pos_len)
    result = np.zeros_like(pos_index, dtype=np.float16)
